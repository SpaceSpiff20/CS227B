var manager = "manager";
var player = "hail_mary_minimax_ab_intermediate_reward";
...
//==============================================================================
// minimax with alpha-beta pruning + intermediate reward evaluation
//==============================================================================
...
function playminimax(role) {
  ...
  if (actions.length === 0) {
    console.log("[minimax_ab_intermediate_reward] no legal actions");
    return false;
  }
  if (actions.length === 1) {
    console.log("[minimax_ab_intermediate_reward] only one action, returning immediately");
    return actions[0];
  }
  ...
  pruned = 0;
  var timedOut = false;
  console.log("[minimax_ab_intermediate_reward] searching " + actions.length + " actions, deadline in " + (playclock - 1) + "s");
  for (var i = 0; i < actions.length; i++) {
    ...
    if (newscore === 100) {
      console.log("[minimax_ab_intermediate_reward] found winning move at action " + i + ", nodes=" + nodes + " terminals=" + terminals + " pruned=" + pruned);
      return actions[i];
    }
    if (newscore > score) {
      action = actions[i];
  ...
  console.log("[minimax_ab_intermediate_reward] done — nodes=" + nodes + " terminals=" + terminals + " pruned=" + pruned + " bestScore=" + score + (timedOut ? " (timed out after " + i + "/" + actions.length + " actions)" : ""));
  return action;
}
...
function intermediateRewardEval(role, state) {
...
function minimax(role, state, alpha, beta) {
  nodes = nodes + 1;
  if (Date.now() > deadline) {
    return intermediateRewardEval(role, state);
  }
  if (findterminalp(state, library)) {
    return intermediateRewardEval(role, state);
  }
